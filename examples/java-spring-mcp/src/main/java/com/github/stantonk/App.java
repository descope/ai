...
<<<<<<< HEAD
            ...
            // Only allow OAuth discovery endpoints without authentication
            if (requestUri.contains("/.well-known/")) {
                if (authHeader == null || authHeader.trim().isEmpty()) {
                    log.info("[{}] OAuth discovery endpoint - allowing request without authentication", requestId);
                    mcpTransport.service(request, response);
                    log.info("[{}] OAuth discovery request completed", requestId);
                    return;
                }
            }
            ...
            if (authHeader == null || authHeader.trim().isEmpty()) {
                log.warn("[{}] Authentication required but no Authorization header provided", requestId);
...
<<<<<<< HEAD
            try {
                ...
                // Set auth info for current thread
                TokenUtils.setAuthInfo(authHeader, userId);
