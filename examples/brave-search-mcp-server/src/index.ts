...
export class MyMCP extends McpAgent<Bindings, State, Props> {
	...
	async init() {
		...
		const performWebSearch = async (query: string, count = 10, offset = 0) => {
			checkRateLimit();
		...
		const performLocalSearch = async (query: string, count = 5) => {
			checkRateLimit();
...
`;
		...
		this.server.tool(
			...
			async ({ query, count, offset }) => ({
				content: [
					{ type: "text", text: await performWebSearch(query, count, offset) },
				],
			}),
		);
		...
		this.server.tool(
			...
			async ({ query, count }) => ({
				content: [
					{ type: "text", text: await performLocalSearch(query, count) },
				],
			}),
		);
	}
